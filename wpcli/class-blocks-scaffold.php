...
class Blocks_Scaffold {
	 ...
	 * : Block Namespace.
	 * : Default: demchco\blocks
	 *
	 * ## EXAMPLES
	 *
	 * wp abs create_portable_block myblock --title="This is myblock" --desc="This block is used for wds." --keywords="myblock" --icon="table-row-before" --namespace="demchco\blocks"
	 * @since  2.0.0
	...
	private function create_block_dir() {
		$dir = DEMCHCO_ROOT_PATH . 'src/blocks/' . $this->name;
	...
	private function create_block_render_php( $args ) {
		$local_file = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/block.php';
		$content    = '';
		...
		if ( $this->init_filesystem()->exists( $local_file ) ) {
			...
			if ( ! $this->init_filesystem()->put_contents( DEMCHCO_ROOT_PATH . 'src/blocks/' . $this->name . '/block.php', $content ) ) {
	...
	private function create_block_tailwind_config() {
		$dir     = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/tailwind.config.js';
		$content = '';
	...
	private function create_block_json( $args ) {
		$local_file = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/block.json';
		$content    = '';
		...
		if ( ! $this->init_filesystem()->put_contents( DEMCHCO_ROOT_PATH . 'src/blocks/' . $this->name . '/block.json', $content ) ) {
			WP_CLI::error( 'ERROR :: Could not create a block json file.', true );
		}
	}
	...
	private function create_block_editor_assets() {
		$assets_js  = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/editor.js';
		$assets_css = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/editor.scss';
		...
		// copy editor js.
		if ( ! $this->init_filesystem()->copy( $assets_js, DEMCHCO_ROOT_PATH . 'src/blocks/' . $this->name . '/editor.js' ) ) {
	...
	private function create_block_assets() {
		$assets_js  = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/script.js';
		$assets_css = DEMCHCO_ROOT_PATH . 'wpcli/block-starter/style.scss';
		...
		// copy editor js.
		if ( ! $this->init_filesystem()->copy( $assets_js, DEMCHCO_ROOT_PATH . 'src/blocks/' . $this->name . '/script.js' ) ) {
			WP_CLI::error( 'ERROR :: Could not create editor js file.', true );
		}
